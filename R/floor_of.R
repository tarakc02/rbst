...
floor_of.bst <- function(tree, key) {
    ...
    res <- floor_of(tree$root, key, tree$compare)
    if (is.null(res)) return(NULL)
    else return(res$key)
}

floor_of.bstnode <- function(node, key, compare) {
    comp <- compare(key, node$key)
    if (comp == 0) return(node)
    if (comp < 0) return(floor_of(node$left, key, compare))
    temp <- floor_of(node$right, key, compare)
    if (!is.null(temp)) return(temp)
    else return(node)
}

floor_of.NULL <- function(node, key, compare) NULL